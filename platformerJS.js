...
// Enemy is initialized based on the character, but otherwise has a
// size and position
...
DOMDisplay.prototype.scrollPlayerIntoView = function() {
  ...
  //1/4 or higher for higher aspect ratios
  var margin = width / 4;
...
Player.prototype.moveX = function(step, level, keys) {
  ...
  // Handle obstacle by calling playerTouched
  if (obstacle)
...
//Edit here to change win/lose conditions.
Level.prototype.playerTouched = function(type, actor) {
  ...
  else if (type == "drip" && this.status == null){
	  this.status = "lost";
	  this.finishDelay = 1;
  }
  ...
  else if (type == "horizontal" && this.status == null){
	  this.status = "lost";
	  this.finishDelay = 1;
  }
  else if (type == "coin") {
...
function Rain(pos, ch){
	...
	this.size = new Vector(1, 1);
	if (ch == "v") {
		// Rain Enemy. Repeat back to this pos.
...
Drip.prototype.type = "drip";
Drip.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  else if (this.repeatPos)
